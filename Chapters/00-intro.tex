...
% Pues di quienes y cítalos... pero por mucho que sea de gente varia eso no es justificación para nada - JJ FERGU: no, no quiero decir que me haya copiado de ellos el texto, me he fijado en como estructuran sus tesis, que primero ponen un párrafo del área que van a tratar en la tesis (ejemplo EAs P2P de juanlu).
...
\section{Objectives} %¿Después de explicar los %algoritmos evolutivos? ¿Por qué? - JJ FERGU: canterior                                
\label{sec:intro:motivation}

% Una tesis no "propone" cosas, PRUEBA algo. ¿Qué estás probando? ¡Esa
% es la tesis! - JJ FERGU: pero esto es la motivación, no el objetivo. Para mí la motivación es algo como "¿Cómo podemos ayudar a la gente?" Comentado todo de todas formas. También era para distinguir el concepto del "objetivo" de la tesis con los distintos sub-objetivos (1 al 4).
...
%the development of EAs to address some of the problems % which
                                ...
                                % ¡Sé más preciso! - JJ
%previously
%explained, such as the  . %Also, other objective of this thesis is to encourage to EA
...
% probar ALGO. - JJ FERGU: quitado. No era un objetivo, era "literatura filosófica", rollo "motivacional"...

%FERGU: A estas alturas de la vida no sé la diferencia entre motivación y objetivos TERMINAR

\lettrine{T}{he} objectives this thesis tries to achieve are:
...
% FERGU: He estado usando tu issue del github para definir los objetivos "Una tesis, en este contexto, trata de probar que a) usar un marco determinado es el más adecuado para solucionar problemas actuales en algoritmos evolutivos b) que OSGi es la tecnología más adecuada para hacer esto c) que Osgiliath es la herramienta más adecuada para implementar ese marco para resolver esos problemas d) que con Osigiliath se pueden, efectivamente, resolver esos problemas y e) que lo estás haciendo". Creo que tu primer objetivo del issue (a) pega más. Es decir "hay entornos que resuelven problemas" es mejor inicio que "se pueden migrar algoritmos a unos entornos" ¿por qué esos entornos? Vamos, prefiero decir "Hay entornos que resuelven" y _despues_ los EAs se pueden migrar a esos entornos y aprovechar sus ventajas. Mira a ver si el nuevo objetivo es mejor que el que tú me propones.
...
% Propose a methodology that is able to successfully adapt evolutionary algorithms to distributed, dynamic, standards-based environment. FERGU: sí, este sí está mejor como tú dices (es como el paso siguiente al anterior) Por cierto, he añadido "heterogeneous"
...
% Validate the methodology using SOA FERGU: no puede ser usando "using SOA", SOA es parte de la metodología (la metodología propone crear servicios y se basa en SOA para definirlos y usa conceptos SOA). SOA es el "entorno" final que queremos. Lo que podemos poner es "using a SOA technology"
...
% y te faltaría: prove that a SOA-based implementation of distributed, dynamic, standards-based evolutionary algorithms is able to solve efficiently this and that problem. - JJ FERGU: lo meto, pero aún tengo duda

%\newcommand{\objectiveresearch}{Solve different problems using SOA-EA and OSGiLiath in several applications}
\newcommand{\objectiveresearch}{Prove that a SOA-based implementation of distributed, dynamic, standards-based evolutionary algorithms is able to solve efficiently different problems} %FERGU: OJO!! Añadir el problema de GP?
\subsection*{Objective 4: \objectiveresearch}
\label{subsec:intro:obj:applications}
The final objective of this thesis is to demonstrate that SOEAs can be used to obtain relevant scientific results in several fields. The methodology and framework will be used to perform experiments in dynamic parameter adaptation to hardware, and creation of competitive bots for video-games. %FERGU: REESCRIBIR ESTO SEGUN LO QUE SE DECIDA
...
motivations and questions to address. % Y una intro a los algoritmos
                                % evolutivos que no viene a cuento - JJ FERGU: afun, no habrá que decir antes cómo está el mundo para justificar lo nuestro?
...
% AGHHH. Estás haciendo lo mismo de antes, pero parafraseándolo. El "paso" no es describir "la clasificación tradicional" de los EAs. la tesis debe ir encaminada a probar algo  cumpliendo los objetivos planteados arriba. ¿Por qué describes eso? ¿qué objetivo estás cumpliendo? Así es como debes engarzar un capítulo con otro. - JJ FERGU: reescrito ENTERO.
...
specific technology. %Two different SOA technologies (Web Services and
%OSGi) are compared. % why two? there are no other? Esto qué valida? La
                    % comparación es para validar? - JJ 
To show the automatic binding of services, an experiment that
adaptively enables and binds services to increase the performance of a SOEA % which performance? FERGU: añadido la del SOEA
...
In the next chapter (Chapters \ref{chap:rts}) OSGiLiath will be used to create SOEAs to generate competitive bots for RTS games. SOA-EA will be applied to create new dynamic services, obtaining relevant results. %FERGU: ENLAZAR ESTO CON UN NUEVO OBJETIVO O CON EL ANTERIOR
...
% una tesis no se desarrolla, ¡se prueba! - JJ FERGU: me refería a desarrollar "el libro". Una tesis no solo se prueba, también se escribe. Meto "probar los objetivos"
...
% ESto ya no corresponde a lo de arriba. FERGU: cambiar!!!