...
Thinking as abstractly as possible requires to separate concepts such
...
is needed. That is the reason we should separate the concept {\em
  recombine} from {\em crossover} (and also, following the top-down
design proposed in SOMA).  % ¿Esto es lo único que estás usando del
                           % SOMA? - JJ

Finally, we must not make assumptions about services previously
executed or being executed next. For example, services such {\em
...
The services developed must match with the following technological
restrictions: % ¿por qué? ¡Justifícalo todo! - JJ
\begin{itemize}
\item These services can dynamically bound %bind o be bound - JJ
 to change the needed EA aspects. 
\item The source code of  the basic EA services must not been % seriously? must not been? - JJ
re-written or re-compiled to achieve this task. 
...
\item Fitness evaluation. Each problem should implement an interface
  ...
  distribution of this service (instead of being a method in the {\em
    individual} class, for example). However, to be more flexible, the
  {\em fitness service} must receive a list of at least one
  individual, to facilitate the parallelism. % ¿Seguro que hace falta
                                % esto? ¿No hará falta un simple
                                % servicio de selección? Tienes que
                                % justificarlo todo sobre todo si es
                                % parte de TU TESIS - JJ
\item Definition or addition of every type of operator. % ¿definition
                                % or addition? - JJ
...
parameters, for example. % pero ¿esto lo vas a hacer? Recuerda el
                         ...
                         % sea como prueba de concepto)- JJ
\item Adaptation of the evolutionary model. The user can manually
  select the services to be combined to create a Genetic Algorithm or
  an Evolution Strategy, for example. % See above
\item Dynamic adaptation of the parameters. Parameters can also be a
  service, % _returning_ parameters or getting the value of
           % parameters. Los parámetros son valores, no pueden ser un
           % servicio. Usa el lenguaje con propiedad - JJ
 thus the EA developer obtains two advantages: % ¿sobre qué? - JJ
...
\item Flexible output mechanisms. The developers do not need skills in
  ...
  operators. % Esto no lo veo nada claro... Programar un log no es tan
             % complicado - JJ
\end{itemize}
