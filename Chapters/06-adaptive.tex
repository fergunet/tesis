...
\cite{Foster2005Science} are % si se han presentado en los capítulos
...
this motley set as a heterogeneous cluster. % frase guay, pero tienes
...
not complex. % Cita del asynchronous EAs del PPSN de Dortmund - JJ
 Also, in that chapter, % where? - JJ
 we presented the work of \person{Alba \etal}
 \cite{Alba2002Heterogeneous}, where dEAs with the same parameter
 configuration are even % cita el capítulo, no el trabajo!!! - JJ FERGU: arreglado
...
each island. % ¿Por qué? ¿Cómo? ¿Cómo se relaciona esto con LA TESIS?
             ...
             % hecho como un hecho consumado. - JJ


In this chapter, OSGiliath has been used % ¿Se podía haber usado otra
...
mixed with the parents and the worst individuals are removed. % 1: los
 ...
 problem) is found.   % ¿Pogqué? ¿Pogqué? No puedes soltar un montón
                      ...
                      % sin relacionarlas con LA TESIS) - JJ
...
\cite{SchafferOnemax91}. Both problems have been described previously in Section \ref{sec:osgiliath:experiments}. Each one requires different actions/abilities by the GA
...
% estas cuatro configuraciones? ¿Qué dificultad a priori tienen? - JJ
...
\item HoSi/HoHa: Homogeneous Size/Homogeneous Hardware. The same
  sub-population size in each island on a homogeneous cluster. % hardware y software, ¿no? Deberías decir algo como "system" - JJ
...
\end{itemize} % ¿No falta AdSi HoHa? ¿Por qué no lo has puesto? - JJ

Two different computational systems have been used: a {\em
...
Our aim consists in validating the following hypothesis: adapting the
...
cluster nodes presents an improvement in execution time. % Hombre, ya
                                ...
                                % TESIS? - JJ
In this work, for a possible offline manner, % ein? - JJ
...
\rowcolor{colorCorporativoSuave}Crossover rate & 0.5\\ \hline % muy
                                % bajo probablemente.
\rowcolor{colorCorporativoMasSuave}Mutation rate & 1/individual size\\
...
& 374, 364, 262 and 24 (from N1 to N4)\\ \hline % ¿REsultado? ¿Cómo se
                                % ha obtenido? - JJ
...
% Hombre, para una cosa que nombra Osgiliath, lo quitas - JJ 
...
% para hacer este tipo de experimentos - JJ
...
used. % y esto lo haces porque... ¿Debe todo el mundo saber esto? -JJ
 ...
 In the HeHa system,  offline adaption of  the sub-population to the computational
...
Regarding to AdSi/HeHa, %agh, regarding to. ¡Amoragismos no! - JJ
...
% ¿onemax es el que es eficiente? ¿resolverlo lo es? - JJ
...
in every %¿lo haces tú o la subsección? - JJ
...
%introducción, una línea argumental y una conclusión - JJ