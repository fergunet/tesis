...
\minitoc\mtcskip
\vfill
\lettrine{T}{he} final aim of this thesis is to apply the created framework to solve the problems previously addressed, and therefore, to be used to perform some kind of research. In this chapter, the capabilities of OSGiLiath will be used to investigate if adapting the parameters of a distributed EA taking into account the computational capabilities of the different nodes of execution leads to an increase of performance.
...
this motley set as a heterogeneous cluster. As explained in Chapter \ref{chap:soa} possible way to increase 
the interoperability within these system is Service Oriented Architecture, and specifically, the use of OSGiLiath framework as an example.
...
Also, as explained in Chapter \ref{chap:distributedEAs}, Distributed Evolutionary
...
not complex. % Cita del asynchronous EAs del PPSN de Dortmund - JJ
...
In this chapter, OSGiliath will be used % ¿Se podía haber usado otra
 ...
 to create a heterogeneous distributed system to be used to develop a scientific research related with PATATIN.
...
The aim of this chapter is to use OSGiLiath to demonstrate if adapting the sub-population size to the computational power of an heterogeneous cluster nodes presents an improvement in execution time. New services will be created to TERMINA




\subsection{Algorithm to develop}
This subsection shows the algorithm to develop
...
mixed with the parents and the worst individuals are removed. % 1: los
...
The results should be independent of the problem used, but we have
...
building-blocks mixing. % Podías citar los trabajos de Juanlu en esto
                        % y/o Lobo y/o Carlos Fernandes - JJ
...
cluster nodes presents an improvement in execution time. % Hombre, ya
                                ...
                                % TESIS? - JJ
In this work, for a possible offline way to calculate the computational performance of each node, % ein? - JJ fERGU
...
Once the problem to solve, the algorithm to implement and the different configuration have been described, the restriction to the services to develop are the next:
\begin{itemize}
...
Once the requirements of the system to develop has been presented, the SOA-EA methodology (explained in Chapter \ref{chap:soaea}) is used.

\subsection{Identification}

In addition of the services for calculating the fitness of the problems ({\em MMDPFitnessCalculator} and {\em OneMaxFitnessCalculator}), or the {\em OptimumStopCriterion} and crossovers and mutators created in previous chapter, new services should be added. The first one deals with the migration between islands, so, a service {\em Migrator} (to receive and send individuals from/to other nodes) needs to be created. Also, it is necessary a service to start or stop the remote loops in all islands at the same time (service {\em Launcher}).

\subsection{Specification}

The service {\em Launcher} and its specification {\em ExperimentLauncher} automatically binds all available {\em Algorithm} services in the network. This service can start all the distributed EAs at the same time. When one of them has finished, it has to notify the others to stop.

In this case, each node offers a migration buffer to
...
Once the services have been created, the different combinations of systems and parameter are evaluated.
...
Table \ref{table:parameters} summarizes all the parameters used in the
...
\begin{SCfigure}[htb]
\centering
 \includegraphics[scale =0.4] {gfx/adaptiveresults/generationsMMDPhomosize.pdf}
...
\begin{SCfigure}[htb]
\centering
 \includegraphics[scale =0.4] {gfx/adaptiveresults/generationsMMDPheterosize.pdf}
...
\begin{SCfigure}[htb]
\centering 
\includegraphics[scale =0.4] {gfx/adaptiveresults/generationsONEMAXhomosize.pdf}
...
\begin{SCfigure}[htb]
\centering
\includegraphics[scale =0.4] {gfx/adaptiveresults/generationsONEMAXheterosize.pdf}
...
In this chapter OSGiLiath is used to address a UN QUE. Services for migration and algorithm control have been created to be automatically bound.
...
OSGiLiath has been used to present a study on the adaptation of the sub-population sizes of a distributed EA to the computational power of the different nodes of an heterogeneous cluster. Two adaptation schemes (offline and online) that use information of the computational load of the algorithm have been tested.
