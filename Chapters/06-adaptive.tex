...
%FERGU: muy bien pensado. Joder, qué bien se te dan estas cosas.
...
\begin{itemize}
 \item Can a distributed SOEA be adapted to leverage the capability of a
   heterogeneous cluster?  % ¿Esa son las preguntas de la TESIS?
                           % ¿Dónde lo has dicho? - JJ FERGU: son preguntas DEL CAPÍTULO (puesto) y SOEA
 \item How the adaptation of the sub-population size to the
   computational power affects the execution time and number of
   evaluations?  % Y esta pregunta es interesante porque... - JJ FERGU: abajo
 ...
 \item How is each service of the algorithm (selection, recombination, mutation, replacement and migration) affected by the different
   configurations?  % ¿Y cómo afecta todo esto a LA TESIS? - JJ FERGU: abajo
\end{itemize}
...
amount is 1024). This value has been chosen empirically, as it is big enough to test different sub-population sizes.  % ¿Por qué? - JJ FERGU: puesto.
...
% ¿Ya está? ¿Nada de laxitud pos-tabla? Ahí está, y apáñatelas, ¿no? - JJ FERGU: Esta frase ya no está 
...
correctly. %¿Cuál es la velocidad "real"? - JJ FERGU: puesto (in time)
 However, the number of evaluations has been included in
this chapter to better understanding the results. % ¿Better que qué? - JJ FERGU: better than no ponerlo. En Wordreference dicen que se pone así
The total number of
...
% haz siempre tests estadísticos. - JJ FERGU: sí, los tests están hechos
...
%los outliers o los pones logarítmicos - JJ FERGU: puesto
...
\caption{Average fitness in the first 1000 milliseconds of execution
  ...
  sub-po\-pu\-la\-tion sizes (HeSi/HeHa) for the MMDP problem.} % Explica la
                                % leyenda en este pie. ¿Qué diablos es
                                % n1, n2, etc...? - JJ FERGU: cambiada leyenda a las imagenes
\label{fig:hesiheha}
\end{SCfigure}
...
%¿Media de cuantas? - JJ FERGU: cambiando todo (figuras y pies de pagina)
\label{fig:sizesMMDP}
\end{SCfigure}
...
\caption{Population size in all nodes of the AdSi/HeHa configuration during one execution to solve
  the MMDP problem.} % ¿average? ¿Typical? ¿Por qué un nodo se queda
                     ...
                     % usan o algo - JJ FERGU: cambiada la imagen para que tenga el mismo nombre y el pie de pagina
...
\caption{Average % over what? -proportion over what? What does it
                 % mean¿ - JJ FERGU: after all runs
sub-population size in each node on the heterogeneous cluster with adaptive size (MMDP) after all runs.}
...
\caption{Statistical significance of the results for M\-M\-D\-P.}
...
Furthermore, changing the sub-population size affects to the services (stages)
