...
As stated in Section \ref{sec:soa:whatis}, all services should be seamlessly accessed locally or
...
transmission. OSGi allows several implementations for the service distribution.  % so what? - JJ % ¿Esto es relevante? - JJ FERGU: TODO sí, esto es MUY relevante, lo uso en la tesis, pero antes es que era una sección más grande. Pregunta: menciono aquí que lo voy a usar en mi tesis o en el capítulo Osgiliath?
...
In the previous chapter we described several shortcomings % y dale - JJ FERGU: cambiado a shortcomings
 ...
 motivate the creation of a proper way to define services for evolutionary algorithms. The elements that combine an EA are candidate to be designed as services, as they can behave as input/output functions, allowing the next issues:  
...
\item Services are independent of the used technology or the
  ...
  distributed or local. % no te repitas. Eso ya lo has dicho
                        % antes. Suena a que estás metiendo morralla
                        % por rellenar - JJ FERGU: en realidad estoy diciendo como los servicios resuelven los problemas esos de los EAs, pero lo complemento más
...
\item SOA is the key to develop Open Science
  \cite{Foster2005Science}.% vaya, aquí lo dices, pero antes no - JJ FERGU: sí, está dicho arriba (y citado)
\end{itemize}
 
To allow the benefits that SOA offers in integration, standardization and dynamism, the services for EAs should match with the next technological
restrictions: % ¿por qué? ¡Justifícalo todo! - JJ FERGU: Cambiada la primera frase para justificar lo siguiente
\begin{itemize}
\item The services can be dynamically bound %bind o be bound - JJ FERGU: arreglado
 to change the needed EA aspects. 
\item The source code of  the basic EA services should not be % seriously? must not been? - JJ FERGU: arreglao
re-written or re-compiled to achieve this task. That means that the design must be  
\item New services can be added in execution time. 
\item No specific source code for a distribution must added, neither
  the existing source code of the services should be modified for this
  purpose (that is, changing distribution libraries must not add extra
  code in existing services). 
\end{itemize}
...
\caption{SOA as abstract paradigm to develop in different EAs
  areas. %¿to develop qué? ¿o to be developed? - JJ
...
SOA follows the lines of genericity in software for EAs presented in Section \ref{sec:distributed:design}, but can also extend them:
\begin{itemize}
\item Genericity in the service interfaces: service interfaces are established to create new implementations. Furthermore, these interfaces should be abstract enough to avoid their modification.
\item Loose-coupling: %FERGU: TEEEEERMINAAAA
\item Programming language independence: for example, services implemented in Java may use services implemented in C++ and vice-versa.
...
\item Flexibility: there should be easy to add and remove elements to use the self-adaptation or other mechanisms.
\end{itemize}
