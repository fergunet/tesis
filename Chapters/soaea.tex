...
\item Generic operations: operations should be used in conjunction with others and have minimal side effects. For example, a multi-parent reproduction (crossover with more than two parents) requires separation of the concept {\em Crossover} with only two elements, so the crossover steps should accept a list of individuals. Also, new operations must be created without affect the existent ones, allowing the interaction of a non-limited number several operators. Finally, the granularity of the design of the operators must be equilibrated, not being neither too coarse (may limit the flexibility to create new operators) nor too fine (could be difficult to integrate all the operators).
...
%%%%%%IMPORTANTE PONER REFERENCIA AL CAPITULO 2 CON \REF
...
\item Configurable output: the output must be configurable. This is due to different statistics could be used depending on the EA: for example, the GP tree depth in GP. Also, different outputs (console, files) should be managed.



\end{itemize}

All these characteristics of genericity for frameworks for EAs should be taken into account when designing services for EAs. However, requirements are aligned with the requirements for designing services.
...
Next chapter will use a specific SOA technology (OSGi) to implement the examples shown in previous sections, and how to accomplish the requirements in the development of EAs and SOA, taking advantage of the capabilities of SOA.
