...
Genetic Algorithms (GAs) are a type of optimization algorithms THAT %EIBEN
...
This kind of algorithms can be seen as an hybrid combination of the population-based evolution methods coupled with some kind of local search. Initially, the hybridation was made just combining two o more methods with some kind of problem knowledge. For example genetic local search \cite{}. The local search can performed before, during, or after the evalutation. New trends, such as adaptative MAs \cite{ong2006classification} leads to the usage of several memes for search and decide dynamically which meme to apply to each individual. \person{Ong \etal} propose the next classification for this kind of algorithms:

\begin{description}
\item[Hyperheuristic Adaptive MAs] \person{Cowling \etal} proposed the term \textsc{hyperheuristic} \cite{cowling2001hyperheuristic} as the strategy to choose a meme to apply depending the time and the region of the search space. Or, in a brief, heuristic to choose memes.


\item[Multimemes and Co-Evolving MAs] \person{Krasnogor \etal} \cite{krasnogor2002multimeme} proposes the inclusion of the memetic codification inside the individual chromosome to select wich meme to use, or codify a set of rules, as proposed by \person{Smit} \cite{smith2002co}. 

\item[Meta-Lamarckian Learning] 
\end{description}
