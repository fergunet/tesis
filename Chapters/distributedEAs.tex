...
\lettrine{A}{lthough} the EAs follows the previous scheme showed in Figure \ref{fig:basicscheme}, they have differences depending of the representation of the solutions, the problems to solve, and other features. This section explains the traditional variants, according to \cite{}. 
...
\item[Evolution Strategies] The Evolution Strategies (ES) are used to solve problems whose solution is included in the domain of real numbers, with differences with respect the GAs. The main difference is the inclusion of self-adaptation of the mutation rate, being coded in each individual \cite{eiben2005shared}. Also, the parent selection is performed randomly. ES have been applied in fields such as Evolutionary Robotics \cite{Garcia2012testing}.

\item[Evolutionary Programming] In Evolutionary Programming (EP), the representation of the solution depends of the nature of the problem to solve, for example, use neuronal networks as individuals \cite{}. The mutation is performed after analyze the 
...
Memetic Algorithms (MAs) are based in the concept of \textsc{meme} proposed by \person{Dawkins} \cite{dawkins2006selfish}. A meme is domain specific knowledge coded in a computational representation to effective solving a problem. It is a cultural know-how building-block transmittable and replicable.
...
\lettrine{E}{volutionary} Algorithms are inherentely parallelizable. Fitness evaluation can be distributed into several slave machines, or distribute the population in different nodes to be evolved at the same time. 

\subsection{Traditional parallelization classification}
Two main types of parallelization models for EAs were classified by \person{Alba and Tomassini} in \cite{alba2002parallelism}: \textsc{Global parallel EAs} and \textsc{Spatially structured EAs}.

\subsubsection{Global parallel evolutionary algorithms}%Farming model (centralized EAs) 
...
\subsubsection{Spatially structured algorithms}%Island model (distributed EAs)
\begin{description}
...
There exist a large number of framework for Evolutionary Algorithms available for download. Practically every programming language has his own implementation of the basic elements that form an EA. This implies a large effort made in each one, because these elements not compatible among them. It is also difficult to migrate the code from a framework to another, because the existence is hidden global variables or language specific features (such as lambda-programming). Even as SOA is used extensively in software development, it is not widely accepted in the main EA software.
...
Firstly, there exist
Object Oriented frameworks, such as Al\-go\-rithm::Evo\-lu\-tionary \cite{PERL}, \definicion{METCO}{Metaheuristic-based Extensible Tool for Collaborative Optimization} \cite{METCO},
...
Distributed EAs can improve the algorithmic and computational performance. Classic parallelization approachs, such as the master-slave or island-based models, have been updated with the usage of new trends as P2P or pool-based EAs. These new approachs manage with computational nodes entering and exiting during the experiment runtime and heterogeneous .

And finally, there exist a large number of different (and incompatible) frameworks for EAs.