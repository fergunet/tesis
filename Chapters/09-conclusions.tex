...
In the past decade, and  thanks to the Internet booming, a paradigm change from Object Oriented Programming to Service Oriented Architecture (SOA) has begun. In this paradigm the software is accessed as interoperable services that allow researchers to share data and applications in a remote way. The usage of services does not imply remote accessing, but a way to develop and integrate without assumptions about implementation or the technology used. Evolutionary Computation is a conducive area to migrate to SOA, because this kind of algorithms is inherently configurable and paralellizable. Moreover, there exist many software tools for EAs, although difficult to be integrated. Also, the booming of new trends, like Cloud Computing, and the usual heavy requirements of this kind of algorithms makes them ideal to be transformed into loosely coupled and distributed services for an easy integration. 

\subsection*{Objective 1: \objectiveparadigm}

Chapter \ref{chap:distributedEAs} presented new trends on design of EAs, distribution and parallelization models and frameworks. A number of problems have been identified in the fields of development, integration, interoperability and dynamism

% jolines, ¡ unas conclusiones no son enumera los capítulos de la tesis! AGGGh! - JJ
Chapter \ref{chap:soa} proposed SOA as a possible solution to address these problems. First, SOA fits with the genericity advantages in the development of software for EAs, proposed by \person{Gagn\'e and Parizeau}. It also adds new features, like language independence and  distribution mechanisms. SOA allows the addition and removal of services in execution time without altering the general execution of the algorithm (that is, it is not mandatory to stop it or to add extra code to support new operators). It also increases the interoperability between different software elements (for example, it is possible to add communication libraries without modifying existing code). Thanks to this, the existing EA frameworks could be re-used thanks to SOA, because it provides language independence. Both shortcomings were pointed out by \person{Parejo \etal}. Also, SOA facilitates the code distribution, as it does not require the use of a concrete implementation or library. Finally, this paradigm allows access to already created and operative services, contributing to the growth of Open Science.

\subsection*{Objective 2: \objectivemethodology}

A four step methodology called SOA-EA has been presented in Chapter \ref{chap:soaea}. This methodology, based in SOMA, establish four steps to develop SOEAs: identification, specification, implementation and deployment. 



\subsection*{Objective 3: \objectiveframework}

Loose coupled services for EAs have been designed with the SOA-EA methodology, and they have been implemented inside the OSGiLiath framework (Chapter \ref{chap:osgiliath}).  These services have been combined in several ways to obtain different algorithms (from a canonical GA, a NSGA-II has been created just adding new services). These services have also been dynamically bound to change the needed EA aspects (model and distribution mechanisms among others). The source code of the basic EA services have not been re-written or re-compiled to achieve this task.
...
\subsection*{A methodology to model the services that compose a service oriented EA has been proposed}

The requirements in EA design (genericity in representation, fitness, operations, model, parameters and output), with the requirements in SOA (genericity in interfaces, language independence, distribution and dynamism), have been taken into account to propose the SOA-EA methodology. This methodology proposes 4 iteratively and incremental phases: identification, specification, implementation and deployment. A number of questions has been proposed to answer in each phase to help with the development. This methodology can been used to create SOEAs that takes advantage of the SOA capabilities, such as loose-coupled services and automatic binding of new operators. 

\subsection*{A framework, called OSGiLiath, that accomplish with the restrictions and benefits of using SOA for EAs has been developed}

Using a public specification technology (OSGi) a framework that takes advantages of all the benefits of EAs has been created. This framework uses dynamic binding, remote service distribution and transmission independence. Different modules with different kinds of operators for EAs, problems and elements and have been developed. This framework have been used to demonstrate the advantages of using SOA with different experiments and applications.

\subsection*{Two possible methods for parameter adaptation to hardware have been proposed}
...
online the sub-population sizes has been performed comparing the current generation with the neighbour generation. The results show that adapting the sub-population sizes of a distributed EA to the computational power of each island (node) decreases time to find the optimum of the problem to solve. This is due to the combination of heterogeneous parameter setting and heterogeneous hardware, because no improvement was attained using the heterogeneous parameter set in an homogeneous cluster.

\subsection*{A comparison of using HSV and RGB histograms in Evolutionary Art has been studied}
 Three different fitness functions based on color histogram have been tested: difference between the HSV and
...
A Genetic Programming algorithm that generates agents for playing Planet Wars game have been developed. A number of possible actions to
perform and decision variables have been presented.  The obtained bots have been tested against a larger
set of maps not previously used during the evolution, obtaining
equivalent or better results than competitive bots available in the literature. These two bots
