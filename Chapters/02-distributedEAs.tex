...
%FERGU: He reescrito TODO lo de arriba en lo que hay aquí:
...
Initially the EAs were proposed as a fixed set of steps with different operators and data structures. Then, different ways to parallelize these algorithms were also proposed, with new classifications and operations. Nowadays, there exist new emerging trends that deal with new technologies (such as P2P or Cloud Computing) and new algorithmic methods (such as parameter adaptation) . These trends require a new way to develop EAs BECAUSE.

In this chapter we present the classic classification of EAs to clarify their common elements, together with the most extended models to parallelize these algorithms. This classification can be used as a base for designing new algorithms, establishing their similarities. Then, the new trends in EAs are also presented to summarize their benefits, but also their lacks and problems that should be addressed. Finally, some of the most used frameworks to develop EAs are listed and analysed, to understand their capabilities and lacks. This way, in following chapters, the design of services for EAs will have a solid base to start.
...
Other EAs, that does not match in the previous classification  have been proposed. For example \textsc{Differential Evolution}
...
offer. %La escalabilidad no la da el sistema, sino el algoritmo. Algo
 ...
 An example of EA that has been designed to take advantage of these systems, is
