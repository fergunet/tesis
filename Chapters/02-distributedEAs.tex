\myChapter{Evolutionary Algorithms}\label{chap:distributedEAs} %Distributed o solo EAs? Si es distributed, ¡lo es! - JJ
\minitoc\mtcskip
\vfill
...
This chapter shows a brief revision of the state of the art in this area, showing the most used algorithms, distribution models, and frameworks, to finally explain some of the existent deficiencies to be solved. We also introduce the terminology that will be used in next chapters.  
...
whose design is inspired by the mechanisms of natural evolution %aquí
                                %la obligatoria cita de Darwin - JJ
...
new individuals to be added to the population. % o sea, ¿la población
                                ...
                                % genético - JJ
The general scheme of an EA, extracted from the work of \person{Eiben and Smith} \cite{eiben2010whatis} is described in Figure \ref{fig:basicscheme}.


\begin{SaveVerbatim}{BasicEAtext} %hay un entorno algoritmo que hace
...
BEGIN
 ...
 OD
END
\end{SaveVerbatim} %Parece COBOL. Qué vintage - JJ
...
\lettrine{A}{lthough} EAs follow the scheme shown in Figure
...
features. This section explains the traditional variants, according to
the book of \person{Eiben and Smith} \cite{eiben2003introduction}.
...
\item[Genetic Programming] The objective of this technique is to
  ...
  problems. Individual representation is usually in form of a tree,
  ...
  individuals is usually fixed, to avoid high evaluation costs. GP has
...
Other EAs have been proposed, such as \textsc{Differential Evolution}
(DE) \cite{storn1997differential}, \textsc{Estimation of Distribution
  Algorithms} (EDAs) \cite{larranaga2002estimation} or
\textsc{Bayesian Optimization Algorithms}
\cite{pelikan2005bayesian}. %SEGURO QUE SON EAS? 
% Por supuesto que lo son y no encajan dentro de la rígida
% clasificación anterior. Por no mencionar EAs basados en pool u otro
...
based on the concept of \textsc{meme} % ¿por qué small caps y no
                                % italica? - JJ
proposed by \person{Dawkins}
\cite{dawkins2006selfish}. In this context, a meme is domain specific knowledge coded by a computational representation to the effective solving of a problem. %It is a cultural know-how building-block transmittable and replicable.
...
codify a set of rules, as proposed by \person{Smit} \cite{smith2002co}
(\textsc{Co-evolving MAs}). % y esto es interesante para tu tesis
...
\lettrine{E}{volutionary} Algorithms are inherently parallelizable. % ¿Inherentemente por qué? ¿No querrás decir trivialmente? Además, no es tan trivial porque una vez que metes esquemas de inmigración la cosa no es tan fácil: quien emigra, cuando lo hace, como se incorpora en la población... - JJ
Fitness evaluation can be distributed into several slave machines, % lo que sólo funciona si el trasiego es más rápido que la evaluación local, y sólo para aquellos algoritmos en los que la evaluación de un individuo sólo dependa del mismo: hay esquemas de escalado del fitness o algoritmos coevolutivos en los que esto no tiene por qué funcionar - JJ
...
scalability or decentralization are not taked into account.  % di
...
\subsection{Non-conventional approaches} % ¿Por qué no convencionales?
                                % - JJ
...
``smart dust'' \cite{Rollings2008smartdust} or inside robots, who
learn on-line from the environment \cite{Garcia2012testing}. But with
...
offer. %La escalabilidad no la da el sistema, sino el algoritmo. Algo
 ...
 \definicion{EvAg}{Evolvable Agent} \cite{laredo2010evag} is an EA
 that uses a decentralized population, where each peer has a single
 ...
 their current neighbours. % hala, non-sequitur. ¿Un ejemplo? ¿El más
                           ...
                           % problemas y como? - JJ
The population structure is maintained using the \textsc{newcast}
...
There exist several works that describe EAs being executed
% no uses "There exist" como en el chiste de Gila "Hay alguien que ha
% hecho algo malo..." - JJ
in these systems \citep{grid8,grid4,grid10}. \textsc{Volunteer
...
systems, using different techniques, such as virtualization
...
\lettrine{T}{here} exist % cada vez que dices There exist, $deity mata
                         % a un gatito - JJ
a large number of frameworks for Evolutionary
Algorithms freely available for download. 
...
difficult to migrate the code from a framework to another, mainly duehttp://code.google.com/p/deap/
