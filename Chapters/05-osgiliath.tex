...
Unlike Web Services, OSGi includes a whiteboard pattern for e\-vent-ma\-na\-ger and service control.
   The first means that services can inform what they are doing without indicating 
 ...
 track this number. The whiteboard pattern for service control imply that services are dynamically bound when they appear.
...
Table \ref{tab:osgivsws} summarizes the differences between the two SOA implementation technologies proposed.

\begin{SCtable}[][t]
\resizebox{10cm}{!}{
\begin{tabular}{lll}
\hline
\rowcolor{colorCorporativoSuave}    &  OSGi   & Web Services \\
\hline\hline
\rowcolor{colorCorporativoMasSuave}Primary environment & Local & Web \\
\rowcolor{colorCorporativoSuave}Communication protocol & Variable & SOAP \\
\rowcolor{colorCorporativoMasSuave}Dynamism & White-Board & Explicit \\
\rowcolor{colorCorporativoSuave}Client creation & Previously known & Automatic\\
\rowcolor{colorCorporativoMasSuave}Language & Java (by default) & Any\\
\rowcolor{colorCorporativoSuave}Interfaces & Java Interface & WSDL\\
\rowcolor{colorCorporativoMasSuave}Extras & Plug-in based & WS-Extensions\\ 
\hline
\end{tabular}
}
\caption{Differences between OSGi and Web Services.}
\label{tab:osgivsws}
\end{SCtable}
...
this case only the {\em Replacers} whose property \texttt{re\-pla\-cer\-Na\-me = nsga2} are used. 
...
\hline\hline
\rowcolor{colorCorporativoMasSuave}OSGiLiath               &   612.36 $\pm$ 6.05  & 0.19 $\pm$ 0.018 \\
\rowcolor{colorCorporativoSuave}OSGiLiath (without OSGi)&   613.36  $\pm$ 4.50 & 0.19 $\pm$ 0.022 \\
...
\hline\hline
\rowcolor{colorCorporativoMasSuave}OSGiLiath               &   612.36 $\pm$ 6.05  & 0.19 $\pm$ 0.018 &  10\\
\rowcolor{colorCorporativoSuave}OSGiLiath (without OSGi)&   613.36  $\pm$ 4.50 & 0.19 $\pm$ 0.022 &  103\\
...
The source code of OSGiLiath is available in the web page \url{http://www.osgiliath.org} under a GNU/LGPL license. A\-ppen\-dix \ref{chap:appendixosgiliath} describes the current components available in this framework.
